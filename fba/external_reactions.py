...
def uptake_and_secretion_reactions(model_compounds, compounds, verbose=False):
    ...
    for c in model_compounds:
        if compounds[c].location == 'e' or compounds[c].name == 'Biomass':
            ...
            # us_reaction = reaction.Reaction("EX_" + us_leftside.model_seed_id + "_" + us_leftside.location + "0")
            # but we normally use a different name
            us_reaction = Reaction("UPTAKE_SECRETION_REACTION " + us_leftside.model_seed_id)
